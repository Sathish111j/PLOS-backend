...
class ContextRetrievalEngine:
    ...
    async def get_full_context(self, user_id: UUID, entry_date: date) -> Dict[str, Any]:
        ...
        context = {
            ...
            "day_of_week_pattern": (
                dow_pattern if not isinstance(dow_pattern, Exception) else {}
            ),
            "relationship_state": (
                rel_state if not isinstance(rel_state, Exception) else {}
            ),
            "sleep_debt": sleep_debt if not isinstance(sleep_debt, Exception) else 0.0,
            "activity_patterns": (
                activity_patterns
                if not isinstance(activity_patterns, Exception)
                else []
            ),
            "seven_day_context": (
                seven_day if not isinstance(seven_day, Exception) else {}
            ),
        }
    ...
    async def get_user_baseline(self, user_id: UUID, days: int = 30) -> UserBaseline:
        """
        ...
        baseline = UserBaseline(
            ...
            energy_stddev=(
                statistics.stdev(energy_levels) if len(energy_levels) > 1 else 1.5
            ),
            stress_level=statistics.mean(stress_levels) if stress_levels else 4.0,
            stress_stddev=(
                statistics.stdev(stress_levels) if len(stress_levels) > 1 else 1.5
            ),
        ...
        logger.info(
            f"Calculated baseline: sleep={baseline.sleep_hours:.1f}±{baseline.sleep_stddev:.1f}, "
    ...
    async def get_activity_patterns(
        ...
        return [ActivityImpact.model_validate(impact) for impact in impacts]
