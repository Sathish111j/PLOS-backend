...
"""
    ...
    def _normalize_extraction(
        ...
        for act in raw.get("activities", []):
            ...
            activities.append(
                NormalizedActivity(
                    ...
                    calories_burned=(
                        estimate_calories(canonical_name.lower(), duration)
                        if canonical_name
                        else None
                    ),
                    is_screen_time=(
                        canonical_name.lower() in SCREEN_TIME_ACTIVITIES
                        if canonical_name
                        else False
                    ),
        ...
        if "steps" not in metrics:
            step_match = re.search(r"(\d{3,6})\s*steps", journal_text, re.IGNORECASE)
            if step_match:
                metrics["steps"] = {
