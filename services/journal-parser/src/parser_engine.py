...
class ExtractedData(BaseModel):
    ...
    mood_score: Optional[float] = Field(None, ge=1.0, le=10.0, description="Overall mood score (1-10)")
    energy_level: Optional[float] = Field(None, ge=1.0, le=10.0, description="Energy level (1-10)")
    stress_level: Optional[float] = Field(None, ge=1.0, le=10.0, description="Stress level (1-10)")
    
    # Health metrics
    sleep_hours: Optional[float] = Field(None, ge=0.0, le=24.0, description="Hours of sleep")
    sleep_quality: Optional[float] = Field(None, ge=1.0, le=10.0, description="Sleep quality (1-10)")
    
    exercise_minutes: Optional[int] = Field(None, ge=0, description="Minutes of exercise")
    exercise_type: Optional[str] = Field(None, description="Type of exercise (e.g., running, yoga)")
    exercise_intensity: Optional[str] = Field(None, description="Exercise intensity: low/medium/high")
    
    # Nutrition
    meals_count: Optional[int] = Field(None, ge=0, le=10, description="Number of meals/snacks")
    water_intake: Optional[float] = Field(None, ge=0.0, description="Water intake in liters")
    diet_quality: Optional[float] = Field(None, ge=1.0, le=10.0, description="Overall diet quality (1-10)")
    
    # Work/Productivity
    work_hours: Optional[float] = Field(None, ge=0.0, le=24.0, description="Hours worked")
    productivity_score: Optional[float] = Field(None, ge=1.0, le=10.0, description="Productivity score (1-10)")
    focus_level: Optional[float] = Field(None, ge=1.0, le=10.0, description="Focus level (1-10)")
    
    # Habits
    meditation_minutes: Optional[int] = Field(None, ge=0, description="Minutes of meditation")
    reading_minutes: Optional[int] = Field(None, ge=0, description="Minutes of reading")
    social_interaction: Optional[str] = Field(None, description="Social interaction quality: low/medium/high")
    
    # Meta
    tags: list[str] = Field(default_factory=list, description="Relevant tags/categories")
    key_events: list[str] = Field(default_factory=list, description="Key events mentioned")
...
class JournalParserEngine:
    ...
    async def parse_entry(self, entry_text: str, user_id: Optional[str] = None) -> ParsedJournalEntry:
        """
...
Return the data as JSON matching the schema."""
            ...
            logger.info(f"Successfully parsed entry - extracted {sum(1 for v in extracted_data.values() if v is not None)} fields")
