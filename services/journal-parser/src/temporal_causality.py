...
class TemporalCausalityEngine:
    ...
    async def analyze_temporal_links(
        ...
        conflict_sleep_link = self._analyze_conflict_to_sleep(yesterday, current_data)
        if conflict_sleep_link:
            causality.append(conflict_sleep_link)

        # Link 2: Late work → Next-day fatigue
        work_fatigue_link = self._analyze_work_to_fatigue(yesterday, current_data)
        if work_fatigue_link:
            causality.append(work_fatigue_link)

        # Link 3: Heavy meal → Digestive discomfort
        meal_digestion_link = self._analyze_meal_to_digestion(yesterday, current_data)
        if meal_digestion_link:
            causality.append(meal_digestion_link)

        # Link 4: Exercise → Sleep quality
        exercise_sleep_link = self._analyze_exercise_to_sleep(yesterday, current_data)
        if exercise_sleep_link:
            causality.append(exercise_sleep_link)

        # Link 5: Screen time → Sleep delay
        screen_sleep_link = self._analyze_screen_to_sleep(yesterday, current_data)
    ...
    def _analyze_conflict_to_sleep(
        self, yesterday: Optional[Dict], today: Dict
    ) -> Optional[Dict]:
        """
    ...
    def _analyze_work_to_fatigue(
        ...
        if yesterday_work < 8 and not (
            yesterday_bedtime and "23" in str(yesterday_bedtime)
        ):
            return None
    ...
    def _analyze_meal_to_digestion(
        ...
        heavy_meal = any(
            "heavy" in str(meal).lower()
            or "large" in str(meal).lower()
            or "feast" in str(meal).lower()
        ...
        digestive_issues = any(
            "digest" in str(symptom).lower()
            or "stomach" in str(symptom).lower()
            or "bloat" in str(symptom).lower()
    ...
    def _analyze_screen_to_sleep(
        ...
        late_screen_time = any(
            ...
            and yesterday.get("bedtime")
            and "23" in str(yesterday["bedtime"])
    ...
    def _analyze_conflict_progression(
        self, previous_entries: List[Dict], current_data: Dict
    ) -> Optional[Dict]:
        """
        ...
        return {
            ...
            "is_improving": (states[-1] == "RECOVERY" if states else False),
    ...
    def _analyze_sleep_debt_trajectory(
        ...
        return {
            ...
            "debt_severity": (
                "critical"
                if cumulative_debt < -10
                else (
                    "high"
                    if cumulative_debt < -5
                    else "moderate" if cumulative_debt < -3 else "low"
                )
            ),
        }
    ...
    def _analyze_activity_impacts(
        ...
        for activity in yesterday_activities:
            ...
            impact = self._detect_activity_impact(
                activity_name, yesterday, current_data
            )
    ...
    def _detect_activity_impact(
        self, activity: str, yesterday: Dict, today: Dict
    ) -> Optional[Dict]:
        ...
        if "badminton" in activity_lower or "exercise" in activity_lower:
            ...
            if (
                yesterday_duration >= 30
                and today_sleep_quality
                and today_sleep_quality >= 7
            ):
                return {
                    "activity": activity,
                    "yesterday_duration": yesterday_duration,
