...
class InferenceEngine:
    ...
    def infer_missing_fields(
        ...
        tier2_inferences = self._tier2_inference(explicit_extractions, preprocessed_text)
    ...
    def _infer_energy_level(
        ...
        if "sleep_hours" in explicit:
            ...
            if sleep_hours >= 8:
                ...
                base_energy += 0.5
                factors.append(f"+0.5 from adequate sleep")
                confidence += 0.05
            elif sleep_hours < 6:
                base_energy -= 2.0
    ...
    def _infer_productivity(
        ...
        positive_keywords = ["productive", "focused", "accomplished", "completed", "finished"]
    ...
    def _infer_exercise_intensity(
        ...
        exercise_type = explicit.get("exercise_type", {}).value if "exercise_type" in explicit else "unknown"
        ...
        if any(activity in exercise_type.lower() for activity in high_intensity_activities):
            intensity = "high"
            confidence += 0.10
    ...
    def _tier3_contextual_inference(
        ...
        if "mood_score_estimate" not in explicit and "mood_score" not in explicit:
            ...
            inferred["mood_score"] = FieldMetadata(
                ...
                reasoning=f"Baseline {self.user_baseline.mood_score:.1f}; " + "; ".join(factors) if factors else "From baseline"
            )
    ...
    def _tier4_defaults(
        ...
        if defaults:
            logger.warning(f"Applied {len(defaults)} population defaults (low confidence)")
