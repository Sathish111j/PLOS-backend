...
class ContextAwareGeminiExtractor:
    ...
    async def extract_with_context(
        ...
        prompt = self._build_context_prompt(
            journal_text, user_context, explicit_extractions, inferred_extractions
        )
    ...
    def _build_context_prompt(
        ...
        # System instructions
        prompt_parts.append(
            """You are an intelligent journal entry analyzer for a personal life operating system (PLOS v2.0).
...
"""
        ...
        if baseline:
            ...
            prompt_parts.append(
                f"- Sleep: {baseline.get('sleep_hours', 'unknown')}hrs (±{baseline.get('sleep_stddev', 0):.1f})\n"
            )
            prompt_parts.append(
                f"- Mood: {baseline.get('mood_score', 'unknown')}/10 (±{baseline.get('mood_stddev', 0):.1f})\n"
            )
            prompt_parts.append(
                f"- Energy: {baseline.get('energy_level', 'unknown')}/10\n"
            )
            prompt_parts.append(
                f"- Stress: {baseline.get('stress_level', 'unknown')}/10\n\n"
            )

        # Day-of-week pattern
        if day_of_week_pattern:
            prompt_parts.append(
                f"## TODAY'S TYPICAL PATTERN ({day_of_week_pattern.get('day_name', 'Unknown')})\n"
            )
            if "sleep_baseline" in day_of_week_pattern:
                prompt_parts.append(
                    f"- Typical sleep: {day_of_week_pattern['sleep_baseline']['value']}hrs\n"
                )
            if "mood_baseline" in day_of_week_pattern:
                prompt_parts.append(
                    f"- Typical mood: {day_of_week_pattern['mood_baseline']['value']}/10\n"
                )
            prompt_parts.append("\n")
        ...
        if relationship_state:
            ...
            prompt_parts.append("## RELATIONSHIP STATE\n")
            prompt_parts.append(f"- Current state: {state} (day {days})\n")

            if state in ["CONFLICT", "COLD_WAR", "RECOVERY"]:
                prompt_parts.append(
                    "- IMPORTANT: Relationship stress likely impacting mood, sleep, and energy\n"
                )
        ...
        # Sleep debt
        if sleep_debt > 0:
            prompt_parts.append("## SLEEP DEBT\n")
            prompt_parts.append(f"- Accumulated debt: {sleep_debt:.1f} hours\n")
            if sleep_debt > 5:
                prompt_parts.append(
                    "- HIGH DEBT: Likely impacting mood, energy, stress\n"
                )
            prompt_parts.append("\n")
        ...
        if recent_entries:
            ...
            for i, entry in enumerate(recent_entries[-3:], 1):
                ...
                if entry.get("activities"):
                    prompt_parts.append(
                        f"  Activities: {', '.join(entry['activities'])}\n"
                    )
            prompt_parts.append("\n")
...
"""
    ...
    def _parse_gemini_response(self, response: str) -> Dict[str, FieldMetadata]:
        """
        ...
        try:
            ...
            for field, field_data in data.items():
                ...
                if not (0.0 <= confidence <= 1.0):
                    logger.warning(
                        f"Invalid confidence {confidence} for {field}, clamping to 0.5"
                    )
                    confidence = 0.5
    ...
    async def generate_insights(
        ...
        prompt = self._build_insights_prompt(
            all_extractions, user_context, predictions, health_alerts
        )
