...
class HealthMonitor:
        ...
        if "sleep_hours" in data:
            ...
            # Hypersomnia
                ...
                alerts.append({
                    ...
                    "value": sleep_hours,
                    "threshold": SLEEP_THRESHOLDS["severe_hypersomnia"]["min_hours"],
    ...
    def _check_relationship_health(
        ...
        # Extended conflict
        if state == "CONFLICT" and days_in_state >= CONFLICT_THRESHOLDS["chronic_conflict"]["min_days_in_conflict"]:
            alerts.append({
                "type": "chronic_conflict",
                "level": AlertLevel.HIGH.value,
                "message": f"Conflict state for {days_in_state} days",
                "value": days_in_state,
                "threshold": CONFLICT_THRESHOLDS["chronic_conflict"]["min_days_in_conflict"],
                "recommendation": "Consider relationship counseling or mediation",
            })
        
        # Extended cold war
        elif state == "COLD_WAR" and days_in_state >= CONFLICT_THRESHOLDS["cold_war_extended"]["min_days_in_coldwar"]:
            alerts.append({
                "type": "extended_cold_war",
                "level": AlertLevel.HIGH.value,
                "message": f"Cold War state for {days_in_state} days",
                "value": days_in_state,
                "threshold": CONFLICT_THRESHOLDS["cold_war_extended"]["min_days_in_coldwar"],
        ...
        if len(self.recent_history) >= 7:
            conflict_days = sum(
                1 for entry in self.recent_history[-7:]
...
class TrendDetector:
    ...
    def detect_trends(
        ...
        if abs(slope) < 0.1:
            ...
            direction = "improving" if metric in ["mood_score", "energy_level", "sleep_hours"] else "worsening"
        else:
            direction = "worsening" if metric in ["mood_score", "energy_level", "sleep_hours"] else "improving"
