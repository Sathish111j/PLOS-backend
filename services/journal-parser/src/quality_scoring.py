...
class QualityScorer:
    ...
    def should_request_clarification(
        self, score: float, level: QualityLevel, missing_fields: List[str]
    ) -> tuple[bool, List[str]]:
        """
        ...
        critical_missing = [
            f for f in missing_fields if self.FIELD_IMPORTANCE.get(f, 0) >= 0.07
        ]
...
class ConsistencyChecker:
    ...
    def check_consistency(
        self, extracted_data: Dict[str, FieldMetadata]
    ) -> tuple[bool, List[str]]:
        """
        ...
        if (
            "sleep_hours" in extracted_data
            and "bedtime" in extracted_data
            and "waketime" in extracted_data
        ):
            calculated_sleep = self._calculate_sleep_hours(
                extracted_data["bedtime"].value, extracted_data["waketime"].value
            )
        ...
        if "mood_score" in extracted_data and "activities" in extracted_data:
            ...
            if mood >= 8 and any(
                neg in str(activities).lower()
                for neg in ["conflict", "fight", "argument"]
            ):
