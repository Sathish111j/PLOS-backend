...
class ExplicitExtractor:
    ...
    def _compile_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            # Sleep patterns
            "sleep_hours": re.compile(r'(\d+\.?\d*)\s*(?:hours?|hrs?)\s*(?:of\s*)?sleep', re.IGNORECASE),
            "sleep_direct": re.compile(r'slept\s*(?:for\s*)?(\d+\.?\d*)\s*(?:hours?|hrs?)', re.IGNORECASE),
            
            # Exercise patterns
            "exercise_duration": re.compile(r'(\d+)\s*(?:min|minutes?)\s*(?:of\s*)?(\w+)', re.IGNORECASE),
            "exercise_distance": re.compile(r'(\d+\.?\d*)\s*km\s*(\w+)', re.IGNORECASE),
            
            # Meal patterns
            "meals": re.compile(r'(?:ate|had|breakfast|lunch|dinner|snack).*?(?:eggs?|rice|chicken|dal|roti|bread)', re.IGNORECASE),
            
            # Water intake
            "water": re.compile(r'(\d+\.?\d*)\s*(?:liters?|L)\s*(?:of\s*)?water', re.IGNORECASE),
            
            # Work hours
            "work_hours": re.compile(r'(?:worked|work)\s*(?:for\s*)?(\d+\.?\d*)\s*(?:hours?|hrs?)', re.IGNORECASE),
            
            # Activities
            "badminton": re.compile(r'\b(badminton|bmton|badmint)\b', re.IGNORECASE),
            "programming": re.compile(r'\b(programming|coding|code|dev|developer)\b', re.IGNORECASE),
            "instagram": re.compile(r'\b(instagram|insta|ig)\b', re.IGNORECASE),
            
            # Mood indicators
            "positive_mood": re.compile(r'\b(happy|great|amazing|wonderful|excellent|good|energized|productive|focused)\b', re.IGNORECASE),
            "negative_mood": re.compile(r'\b(tired|exhausted|sad|angry|frustrated|stressed|anxious|bad|terrible)\b', re.IGNORECASE),
            
            # Conflict indicators
            "conflict": re.compile(r'\b(fight|conflict|argument|angry|mad|upset|jealous)\b', re.IGNORECASE),
        }
    
    def extract(self, text: str, preprocessed_data: Dict[str, Any]) -> Dict[str, FieldMetadata]:
        """
        ...
        # Extract sleep
        sleep_data = self._extract_sleep(text, preprocessed_data.get("time_mentions", {}))
        if sleep_data:
    ...
    def _extract_sleep(
        self,
        text: str,
        time_mentions: Dict[str, str]
    ) -> Dict[str, FieldMetadata]:
        """Extract sleep data"""
        extracted = {}
        
        # Direct sleep hours mention
        match = self.patterns["sleep_hours"].search(text) or self.patterns["sleep_direct"].search(text)
        if match:
...
class Preprocessor:
    ...
    def process(self, raw_text: str) -> Tuple[str, Dict[str, Any], Dict[str, FieldMetadata]]:
        """
        ...
        explicit_extractions = self.explicit_extractor.extract(normalized_text, preprocessing_data)
        
        logger.info(f"Preprocessing complete. Extracted {len(explicit_extractions)} explicit fields")
