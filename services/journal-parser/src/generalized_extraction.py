...
class DynamicActivityDetector:
    ...
    ACTIVITY_VERBS = {
        "played", "did", "went", "had", "watched", "read", "studied",
        "worked", "coded", "programmed", "exercised", "ran", "walked",
        "talked", "chatted", "met", "saw", "visited", "ate", "cooked",
        "cleaned", "organized", "planned", "practiced", "learned"
    }
            ...
            for verb in self.ACTIVITY_VERBS:
                if verb in sentence:
                    ...
                    if match:
                        ...
                        if activity_name not in {'a', 'the', 'some', 'my', 'to', 'for', 'with'}:
                            duration = self._extract_duration_near(sentence, activity_name)
                            activities.append({
    ...
    def _detect_app_usage(self, text: str) -> List[Dict[str, Any]]:
        ...
        platforms = [
            ...
            'netflix', 'spotify', 'reddit', 'whatsapp', 'telegram'
        ]
...
class DynamicMoodDetector:
    ...
    POSITIVE_WORDS = {
        'happy', 'great', 'good', 'excellent', 'wonderful', 'amazing',
        'fantastic', 'love', 'enjoy', 'pleased', 'satisfied', 'content',
        'glad', 'delighted', 'thrilled', 'excited', 'grateful', 'thankful'
    }
    
    NEGATIVE_WORDS = {
        'sad', 'bad', 'terrible', 'awful', 'horrible', 'miserable',
        'depressed', 'upset', 'angry', 'frustrated', 'annoyed', 'irritated',
        'worried', 'anxious', 'stressed', 'disappointed', 'hurt', 'lonely'
    }
    ...
    def estimate_mood(self, text: str) -> Tuple[Optional[float], float]:
        ...
        for i, word in enumerate(words):
            ...
            # Score word
            if word in self.POSITIVE_WORDS or word in self.user_vocabulary and self.user_vocabulary[word] > 6:
                positive_count += intensity_multiplier
            elif word in self.NEGATIVE_WORDS or word in self.user_vocabulary and self.user_vocabulary[word] < 5:
