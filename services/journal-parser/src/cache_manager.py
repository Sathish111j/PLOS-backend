...
class MultiLayerCacheManager:
        ...
        if self.redis:
            try:
                redis_data = await asyncio.wait_for(
                    self._redis_get(key), timeout=5.0  # 5 second timeout
                )
                if redis_data:
        ...
        if self.db:
            try:
                db_data = await asyncio.wait_for(
                    self._db_get_baseline(user_id), timeout=2.0  # 2 second timeout
                )
                if db_data:
    ...
    async def get_user_patterns(
        self, user_id: UUID, pattern_type: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        if self.db:
            try:
                db_data = await asyncio.wait_for(
                    self._db_get_patterns(user_id, pattern_type), timeout=2.0
                )
                if db_data:
                    self.memory.set(key, db_data)
                    if self.redis:
    ...
    async def _db_get_patterns(
        self, user_id: UUID, pattern_type: Optional[str]
