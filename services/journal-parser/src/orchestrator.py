...
from .inference_engine import InferenceEngine, ActivityCorrelationEngine
from .relationship_state_machine import RelationshipStateMachine, RelationshipImpactCalculator
from .health_monitor import HealthMonitor, TrendDetector
from .prediction_engine import PredictionEngine
from .gemini_extractor import ContextAwareGeminiExtractor
from .storage_service import JournalStorageService, JournalEventPublisher
...
class JournalParserOrchestrator:
    ...
    def __init__(
        ...
        self.event_publisher = JournalEventPublisher(kafka_producer) if kafka_producer else None
    ...
    async def process_journal_entry(
        ...
        try:
            ...
            preprocessed_text, preprocessing_data, explicit_extractions = self.preprocessor.process(entry_text)
            ...
            logger.info(f"Context retrieved: baseline={baseline is not None}, sleep_debt={sleep_debt:.1f}hrs")
            ...
            logger.info(f"Inference complete: {len(inferred_extractions)} inferred fields")
            ...
            logger.info(f"Gemini extraction complete: {len(gemini_extractions)} AI-extracted fields")
            ...
            if relationship_state:
                ...
                conflict_mentioned = explicit_extractions.get("conflict_mentioned", FieldMetadata(value=False, type=ExtractionType.EXPLICIT, confidence=1.0, source="explicit", reasoning="")).value
                ...
                if new_state:
                    relationship_transition = state_machine.transition_to(new_state, trigger)
                    logger.info(f"Relationship transition: {relationship_transition['from_state']} -> {relationship_transition['to_state']}")
                    ...
                    await self.storage.store_relationship_event(
                        ...
                        to_state=new_state,
                        trigger=trigger,
                        duration_in_previous_state=relationship_transition["days_in_previous_state"]
                    )
                else:
                ...
                logger.debug(f"Applied relationship impact: {len(adjusted_values)} fields adjusted")
            ...
            if self.event_publisher:
                ...
                # Mood event
                if "mood_score" in all_extractions or "mood_score_estimate" in all_extractions:
                    mood_key = "mood_score" if "mood_score" in all_extractions else "mood_score_estimate"
                    await self.event_publisher.publish_mood_event(
                        user_id=user_id,
                        mood_score=all_extractions[mood_key].value,
                        mood_trajectory=predictions.get("mood", {}).get("trajectory", "STABLE"),
    ...
    def _calculate_quality_level(self, extractions: Dict[str, FieldMetadata]) -> QualityLevel:
        """
        ...
        # Count by type
        explicit_count = sum(1 for v in extractions.values() if v.type == ExtractionType.EXPLICIT)
        ai_count = sum(1 for v in extractions.values() if v.type == ExtractionType.AI_EXTRACTED)
        
        # Average confidence
        avg_confidence = sum(v.confidence for v in extractions.values()) / len(extractions)
        
        # Key fields present
        key_fields = ["sleep_hours", "mood_score", "energy_level"]
        key_field_count = sum(1 for f in key_fields if f in extractions or f"{f}_estimate" in extractions)
