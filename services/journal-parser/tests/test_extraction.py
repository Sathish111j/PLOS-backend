...
def test_sleep_prediction_with_debt():
    ...
    assert prediction["predicted_value"] > baseline.sleep_hours  # Should predict more sleep
...
def test_mood_prediction_with_conflict():
    ...
    prediction = engine._predict_mood(sleep_debt=0, relationship_state=relationship_state)
    
    assert prediction["predicted_value"] < baseline.mood_score  # Should predict lower mood
