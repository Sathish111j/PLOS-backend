...
class VectorStore:
    ...
    async def generate_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            logger.error(
                "embedding_generation_failed", error=str(e), text_preview=text[:100]
            )
            raise
    ...
    async def add_knowledge_item(
        ...
        try:
            ...
            point = PointStruct(
                id=self._hash_to_id(knowledge_id), vector=embedding, payload=payload
            )
            ...
            logger.error(
                "add_knowledge_item_failed", knowledge_id=knowledge_id, error=str(e)
            )
            raise
