...
# Type checking import to avoid circular dependency if possible, or simple import
...
class PersistenceService:
    def __init__(self, bucket_service: BucketService, hybrid_service=None):
        ...
        try:
            ...
            if not bucket_id:
                ...
                inbox = await Database.fetch_one(
                    "SELECT id FROM knowledge_buckets WHERE user_id=$1 AND name='Inbox'",
                    user_id,
                )
            ...
            if (
                "metadata" in ingestion_result
                and "files_uploaded" in ingestion_result["metadata"]
            ):
                ...
                for file_path in files:  # e.g. "knowledge-files/user_id/..."
                    ...
                    await Database.execute(
                        file_query, item_id, user_id, file_name, file_path
                    )
            ...
            if self.hybrid_service:
                try:
                    item_data = {
                        ...
                        "tags": ingestion_result.get("tags", []),
                        "item_type": item_type,
                    }
                    await self.hybrid_service.index_item(item_data)
                except Exception as e:
                    logger.error(f"Indexing error (non-fatal): {e}")

            # 6. Log & Return
            logger.info(
                f"Persisted item {item_id} to bucket {bucket_id} ({bucket_plan.get('bucket_name')})"
            )
